...
task :import_csv => :environment do
  ...
  csv_text = File.read(Rails.root.join('lib', 'data', 'invoice_items.csv'))
  csv = CSV.parse(csv_text, :headers => true, :encoding => 'ISO-8859-1')
    csv.each do |row|
      new_invoice_item = InvoiceItem.new
      new_invoice_item.item_id = row["item_id"]
      new_invoice_item.invoice_id = row["invoice_id"]
      new_invoice_item.quantity = row["quantity"]
      new_invoice_item.created_at = row["created_at"]
      new_invoice_item.updated_at = row["updated_at"]
      new_invoice_item.unit_price = row["unit_price"].insert(-3, '.').to_f
